...
public final class OceanModelWorker implements Runnable {
    ...
    public OceanModelWorker(int port) {
        this.port = port;
...
/*        for(int i = 1; i <= imax-imin+1; i++) {
            for(int j = 1; j <= jmax-jmin+1; j++) {
                u[i][j][kuv_old]    = (Double) ts.rd(new TupleTemplate("u", imin+i-1, jmin+j-1, null, kuv_old)).field(3);
                u[i][j][kuv_new]    = (Double) ts.rd(new TupleTemplate("u", imin+i-1, jmin+j-1, null, kuv_new)).field(3);
                v[i][j][kuv_old]    = (Double) ts.rd(new TupleTemplate("v", imin+i-1, jmin+j-1, null, kuv_old)).field(3);
                v[i][j][kuv_new]    = (Double) ts.rd(new TupleTemplate("v", imin+i-1, jmin+j-1, null, kuv_new)).field(3);
                hu[i][j]            = (Double) ts.rd(new TupleTemplate("hu", imin+i-1, jmin+j-1, null)).field(3);
                hv[i][j]            = (Double) ts.rd(new TupleTemplate("hv", imin+i-1, jmin+j-1, null)).field(3);
                eta[i][j][keta_old] = (Double) ts.rd(new TupleTemplate("eta", imin+i-1, jmin+j-1, null, keta_old)).field(3);
                eta[i][j][keta_new] = (Double) ts.rd(new TupleTemplate("eta", imin+i-1, jmin+j-1, null, keta_new)).field(3);
                h[i][j]             = (Double) ts.rd(new TupleTemplate("h", imin+i-1, jmin+j-1, null)).field(3);
                sfx[i][j]           = (Double) ts.rd(new TupleTemplate("sfx", imin+i-1, jmin+j-1, null)).field(3);
                sfy[i][j]           = (Double) ts.rd(new TupleTemplate("sfy", imin+i-1, jmin+j-1, null)).field(3);
            }
        }*/
        ...
        for(Tuple t : uValsOld)
            u[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax][kuv_old] = (Double) t.field(3);
        for(Tuple t : uValsNew)
            u[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax][kuv_new] = (Double) t.field(3);
        for(Tuple t : vValsOld)
            v[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax][kuv_old] = (Double) t.field(3);
        for(Tuple t : vValsNew)
            v[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax][kuv_new] = (Double) t.field(3);
        for(Tuple t : huVals)
            hu[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax] = (Double) t.field(3);
        for(Tuple t : hvVals)
            hv[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax] = (Double) t.field(3);
        for(Tuple t : etaValsOld)
            eta[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax][keta_old] = (Double) t.field(3);
        for(Tuple t : etaValsNew)
            eta[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax][keta_new] = (Double) t.field(3);
        for(Tuple t : hVals)
            h[(Integer)t.field(1)%imax][(Integer)t.field(2)%jmax] = (Double) t.field(3);
...
System.out.println("got other misc vals");
        ...
        while(kuv <= itmax) {
            ...
            if((hPos==HorizontalPosition.LEFT) || (hPos==HorizontalPosition.CENTRE)) {
                for(int j = 1; j < u[0].length; j++)
                    tempVals.addElement(new Tuple("u", imax, j+jmin-1, u[imax-imin+1][j][kuv_new], kuv));
            }
            if((vPos==VerticalPosition.BOTTOM) || (vPos==VerticalPosition.CENTRE)) {
            ...
            if((hPos==HorizontalPosition.RIGHT) || (hPos==HorizontalPosition.CENTRE)) {
                /*for(int j = 1; j < u[0].length; j++)
                    u[1][j][kuv_new] = (Double) ts.rd(new TupleTemplate("u", imin, j+jmin-1, null, kuv)).field(3);*/
                //Vector<Tuple> uVals = ts.rdAll(new TupleTemplate("u", imin, null, null, kuv), imax-imin);
                Vector<Tuple> uVals = ts.rdAll(new TupleTemplate("u", imin, null, null, null), imax-imin);
                for(Tuple t: uVals) {
                    u[1][(Integer)t.field(2)%jmax][kuv_new] = (Double) t.field(3);
                }
            }
            if((vPos==VerticalPosition.TOP) || (vPos==VerticalPosition.CENTRE)) {
                /*for(int i = 1; i < u.length; i++)
                    v[i][1][kuv_new] = (Double) ts.rd(new TupleTemplate("v", i+imin-1, jmin, null, kuv)).field(3);*/
                //Vector<Tuple> vVals = ts.rdAll(new TupleTemplate("v", null, jmin, null,kuv), jmax-jmin);
                Vector<Tuple> vVals = ts.rdAll(new TupleTemplate("v", null, jmin, null,null), jmax-jmin);
                for(Tuple t : vVals) {
                    v[(Integer)t.field(1)%imax][1][kuv_new] = (Double) t.field(3);
                }
            }
            Profiler.end("io");
            ...
            if((hPos==HorizontalPosition.LEFT) || (hPos==HorizontalPosition.CENTRE)) {
                /*for(int j = 1; j < u[0].length; j++)
                    eta[imax-imin+1][j][keta_new] = (Double) ts.rd(new TupleTemplate("eta", imax, j+jmin-1, null, keta)).field(3);*/
                //Vector<Tuple> etaVals = ts.inAll(new TupleTemplate("eta", imax, null, null, keta), imax-imin);
                Vector<Tuple> etaVals = ts.inAll(new TupleTemplate("eta", imax, null, null, null), imax-imin);
                for(Tuple t : etaVals) {
                    eta[imax-imin+1][(Integer) t.field(2)%jmax][keta_new] = (Double) t.field(3);
                }
            }
            if((vPos==VerticalPosition.BOTTOM) || (vPos==VerticalPosition.CENTRE)) {
                /*for(int i = 1; i < u.length; i++)
                    eta[i][jmax-jmin+1][keta_new] = (Double) ts.rd(new TupleTemplate("eta", i+imin-1, jmax, null, keta)).field(3);*/
                //Vector<Tuple> etaVals = ts.inAll(new TupleTemplate("eta", null, jmax, null, keta), imax-imin);
                Vector<Tuple> etaVals = ts.inAll(new TupleTemplate("eta", null, jmax, null, null), imax-imin);
                for(Tuple t : etaVals) {
                    eta[(Integer) t.field(1)%imax][jmax-jmin+1][keta_new] = (Double) t.field(3);
                }
            }
            Profiler.end("io");
    ...
    public static void main(String[] args) {
        if(args.length != 1) {
            System.out.println("Usage: java apps.oceanModel.OceanModelWorker <PORT>");
            System.exit(1);
        }

        int port = Integer.parseInt(args[0]);

        new OceanModelWorker(port).run();
    }
}
